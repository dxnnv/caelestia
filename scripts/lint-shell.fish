...
if not argparse -n lint-any-shell -X 0 \
    's/shell-dir=' \
    'only-check' \
    'no-cpp' \
    'h/help' \
...
if set -q _flag_shell_dir
  if not test -d "$_flag_shell_dir"
    echo "Shell directory does not exist: $_flag_shell_dir" >&2
    exit 2
  end

  set -g shell_dir (realpath "$_flag_shell_dir")
end

if not test -f "$shell_dir/CMakeLists.txt"; or not test -f "$shell_dir/scripts/qml-lint-conventions.py"
  echo "Not a Caelestia shell checkout: $shell_dir" >&2
  exit 2
end
...
function check_qml
  ...
  for file in (string match -rv "^build(-lint)?/" **.qml)
...
function lint_qml
  ...
  set -l qml_files (string match -vr '(build|build-lint|modules/controlcenter)/.*' **.qml)
